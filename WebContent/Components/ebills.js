...
function onEbillSaveComplete(response, status) {
  if (status == 'success') {
    ...
    $('#alertError').text('Unknown error occurred');
    $('#alertError').show();
  }
...
//UPDATE
//to identify the update button used a class
$(document).on('click', '.btnUpdate', function (event) {
  ...
  $('#ebillid').val($(this).closest('tr').find('td:eq(0)').text());
  ...
  $('#enddate').val($(this).closest('tr').find('td:eq(4)').text());
  $('#refno').val($(this).closest('tr').find('td:eq(5)').text());
});
